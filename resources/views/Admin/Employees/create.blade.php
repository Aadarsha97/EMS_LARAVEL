...
<h2 class="text-2xl font-bold p-6">Add Employee<p< /h2>
        ...
        <form action="{{ route('employee.store')  }}" method="post" class="m-12">
            ...
            <x-forminput name="contact_number" label="Enter Employee Contact" type="number" />
            ...
            <div class="my-2">
                ...
                <select id="name" class="block mt-1 w-full" name="role_id">
                    ...
                    @foreach ($departments as $department)
                    <option value="{{ $department->id }}">{{ "Senior" }}</option>
                    @endforeach
            ...
            <x-forminput name="dob" label="Enter Employee DOB" type="date" />
            ...
            <div class="">
                <button class="bg-blue-500 text-white px-3 py-1 rounded-sm my-2 w-[100%]" type="submit">Add Employee</button>
            </div>
